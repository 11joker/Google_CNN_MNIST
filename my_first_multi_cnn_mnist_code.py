...
MODEL_DIR = r"C:\Users\25493\Documents\GitHub\Google_CNN_MNIST\multi_cnn_mnist"
...
#-----------------------------------第一层卷积-最大池---------------------------------------------------------------
    with tf.variable_scope("cnn_layer1"):
        conv1_weights = tf.get_variable("conv_weights",
            [CONV1_SIZE, CONV1_SIZE, CHANNEL, CONV1_DEEP],
            initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv1 = tf.nn.conv2d(x, conv1_weights, strides=[1,1,1,1], padding="SAME")
        
        max_pool1 = tf.nn.max_pool(conv1, ksize=[1,2,2,1], strides=[1,2,2,1], padding="SAME", name="max_pool")
#------------------------------------第二层卷积-最大池--------------------------------------------------------------
    with tf.variable_scope("cnn_layer2"):
        conv2_weights = tf.get_variable("conv_weights", 
                    [CONV2_SIZE, CONV2_SIZE, CONV1_DEEP, CONV2_DEEP],
                    initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv2 = tf.nn.conv2d(max_pool1, conv2_weights, strides=[1,1,1,1], padding="SAME")
        max_pool2 = tf.nn.max_pool(conv2, ksize=[1,2,2,1], strides=[1,2,2,1], padding="SAME", name="max_pool")
#------------------------------------第一层前馈神经网络--------------------
    ...
    with tf.variable_scope("fwn_layer1"):
        fnn1_weights = tf.get_variable("fnn_weights",
                    [nodes,FNN1], 
                    initializer=tf.truncated_normal_initializer(stddev=0.1))
        fnn1 = tf.matmul(reshaped, fnn1_weights, name="fnn_op")
        
        bias1 = tf.get_variable("bias",
                [FNN1],
                initializer=tf.constant_initializer(0.1))
        relu1 = tf.nn.relu(fnn1 + bias1,name="relu")
#---------------------------------------第二层前馈神经网络------------------------
    with tf.variable_scope("fwn_layer2"):
        fnn2_weights = tf.get_variable("fnn_weights",
                [FNN1,OUTPUT],
                initializer=tf.truncated_normal_initializer(stddev=0.1))
        fnn2 = tf.matmul(relu1, fnn2_weights, name="fnn_op")
        bias2 = tf.get_variable("bias",
                [OUTPUT],
                initializer=tf.constant_initializer(0.1))
        relu2 = tf.nn.relu(fnn2+bias2,name="relu")
#--------------------------------------------
    return relu2
